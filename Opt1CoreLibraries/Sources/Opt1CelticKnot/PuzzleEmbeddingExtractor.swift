...
enum PuzzleEmbeddingExtractor {
    ...
    static func colorEmbeddingV4(for image: CGImage, side: Int = 32) -> [Float]? {
        ...
        for i in 0..<(side * side) {
            ...
            gray[i] = gy
            // Python computes gray_u8 = (gray * 255.0).astype(np.uint8); mirror it here
            // (rather than using the raw luminance byte) so the LBP codes match Python bit-for-bit.
        ...
        // (1) Fine 3x3 spatial HOG (8 bins x 9 cells = 72 dims).
        // Uses the same integer partitioning as the Python reference so cell
        ...
        // (2) Rotation-invariant uniform LBP (10 dims).
        let lbpHist = lbpRotationInvariantUniformHistogram(grayU8: grayU8, side: side)

        // (3) Per-quadrant HSV means (12 dims: TL,TR,BL,BR × (H,S,V)).
    ...
    private static func lbpRotationInvariantUniformHistogram(grayU8: [UInt8], side: Int) -> [Float] {
        ...
        for y in 1..<(side - 1) {
            for x in 1..<(side - 1) {
                ...
                var code: UInt16 = 0
                // Ordering mirrors the Python reference (N, NE, E, SE, S, SW, W, NW).
